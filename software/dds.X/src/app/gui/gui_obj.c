 ...
 * @version based on  0.4b (modification not checked) 2014-04-05
 * @date (yyyy-mm-dd)
...
static pGuiUsrTask_t pUserTask = NULL, pHookUserTask = NULL;  /*pointer to current user handler / hook handler*/
...
/**
 * @function GUI_SetHookUserTask
 * @brief Define a new hook task, which will be executed instead of pUserTask
 * @param pGuiUsrTask_t pFunction: new hook task
 * @return none
 */
void GUI_SetHookUserTask(pGuiUsrTask_t pFunction) {
  pHookUserTask = pFunction;
}
 ...
 * @param signal_t signal: sig
 * @return none
 */
void GUI_SetSignal(event_e event, signal_t sig) {
  if(lastAddedObj != NULL && event < E_MAX_EVENT) {
    lastAddedObj->signals[event] = sig;
  }
}
...
void GUI_DrawObjects(void) {
  ...
  if(pInternalTask == NULL) {
    if(pHookUserTask != NULL) pHookUserTask(signal);
    else if(pUserTask != NULL) pUserTask(signal);
  }
}
 ...
 * @return none
 */
static void HandleTouchEvent(coord_t _x, coord_t _y, g_obj_st *obj) {

  bool inClip, isTouch;

  isTouch = (_x >= 0 && _y >= 0) ? true: false;
  inClip = P2D_IsInClip( &(obj->rec), _x, _y);
