 ...
 * @version based on 0.2b (modification not checked) 2014-04-05
 * @date (yyyy-mm-dd)
...
/**
 * @function GUI_W_ButtonShade
 * @brief shade / unshade a button
 * @param void *_g_obj: button ptr
 * @param bool shade: if true, picture of the button will be displayed in black & white
 * @return none
 */
void GUI_W_ButtonShade(void *_g_obj, bool shade) {
  ...
  if(_g_obj != NULL) {
    ...
    if(g_obj->draw == ButtonDraw) {
      button = (button_st*) g_obj->obj;
      if(button->shade != shade) {
        button->shade = shade;
        GUI_ObjSetNeedRefresh(_g_obj, true);
      }
    }
  }
}
...
static void ButtonDraw(void *_g_obj, void *_obj) {
  ...
  if(_g_obj != NULL && _obj != NULL) {
    ...
    /*background*/
    if(button->shade == true && type <= 1) type += 2; //FIXME: too dirty
    ...
    /*sprite*/
    SetLut(button->shade == true? G_LUT_DISABLED: G_LUT_NORMAL);
