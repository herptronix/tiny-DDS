...
g_obj_st /*@null@*/ *GUI_W_RotaryButtonAdd(const rect_st *rec, int8_t *pVar, gr_enum_e granularity) {
  ...
  if(rec != NULL && rec->w == rec->h && rec->w >= min_w && pVar != NULL) {
    ...
    if(g_obj != NULL) {
      ...
      /*overwrite parameters (following the rotary button picture)*/
      g_obj->rec.x = rec->x;
      g_obj->rec.y = rec->y;
      g_obj->rec.w = g_obj->rec.h = SpriteGetWidth(G_RBTN_000); /*force w&h to the sprite dimension*/
      granularity = ROTARY_BTN_GR_30_DEG;                       /*each frame of the button is rotated of 30°*/
      ...
      if(rbtn != NULL) {
        ...
        /*compute coords*/
        rbtn->rad = g_obj->rec.w / 2 - 1;
        rbtn->xc = g_obj->rec.x + rbtn->rad;
        rbtn->yc = g_obj->rec.y + rbtn->rad;
...
static void RotBtnDraw(void *_g_obj, void *_obj) {
  ...
  if(_g_obj != NULL && _obj != NULL) {
    ...
    Sprite(g_obj->rec.x, g_obj->rec.y, G_RBTN_000 + rbtn->deg / 30);
    P2D_SetDisplayMode(DISPLAY_SOLID);
  }
}
